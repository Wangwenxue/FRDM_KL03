...
void time_delay_ms(uint32 count_val)
{
  ...
  LPTMR_PSR = LPTMR_PSR_PCS(1)|LPTMR_PSR_PBYP_MASK;

  /* Start the timer */
  LPTMR_CSR |= LPTMR_CSR_TEN_MASK;

  /* Wait for counter to reach compare value */
  while (!(LPTMR_CSR & LPTMR_CSR_TCF_MASK));
...
void lptmr_init(int count, int clock_source)
{
    ...
    LPTMR_PSR = ( LPTMR_PSR_PRESCALE(0) // 0000 is div 2
                 | LPTMR_PSR_PBYP_MASK  // clock feeds directly to LPT
                 | LPTMR_PSR_PCS(clock_source)) ; // use the choice of clock
              
    LPTMR_CMR = LPTMR_CMR_COMPARE(count);  //Set compare value
	// clear flag
	LPTMR_CSR |=  LPTMR_CSR_TCF_MASK; 
	
    LPTMR_CSR =(  LPTMR_CSR_TCF_MASK   // Clear any pending interrupt
                 | LPTMR_CSR_TIE_MASK   // LPT interrupt enabled
                 );
	
    LPTMR_CSR |= LPTMR_CSR_TEN_MASK;   //Turn on LPT and start counting
